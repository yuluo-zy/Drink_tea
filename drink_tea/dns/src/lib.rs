...
mod tests {
    ...
    use crate::{ConfigureResolver, Resolver, ResolverOpts};
    use std::str::FromStr;
    use tokio_test;
    use tracing::debug;
    ...
    impl ConfigureResolver for DnsTestConfig {
        fn configure_resolver(&self, _: &mut ResolverOpts) {}
    }
    ...
    pub async fn dns_test_entry() {
        let dns_data: &str = "www.baidu.com";
        if let Ok(name) = dns_data.parse::<Name>() {
            let fcon = DnsTestConfig {};
