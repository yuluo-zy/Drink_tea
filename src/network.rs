...
pub async fn serve(port: u16, secret: String, dns: IpAddr) {
...
mod tests {
    ...
    fn integration_test() {
        ...
        let _server =
            thread::spawn(move || serve(8964, "password".parse().unwrap(), "8.8.8.8".parse::<IpAddr>().unwrap()));
        ...
        let _client = thread::spawn(move || connect("127.0.0.1".parse().unwrap(), 8964, false, "password".parse().unwrap()));
